    ...
    public class OLVExporter {
        ...
        public void Convert() {
            ...
            if (this.IncludeColumnHeaders) {
                ...
                WriteOneRow(sbText, strings, "", "\t", "", null, columnFormats);
                WriteOneRow(sbHtml, strings, "<tr><td>", "</td><td>", "</td></tr>", HtmlEncode, columnFormats);
                WriteOneRow(sbCsv, strings, "", ",", "", CsvEncode, columnFormats);
            }
            ...
            foreach (object modelObject in this.ModelObjects) {
                ...
                foreach (OLVColumn col in columns)
                {
                    ...
                    columnFormats.Add(col.TextCopyFormat);
                }

                WriteOneRow(sbText, strings, "", "\t", "", TextEncode, columnFormats);
                WriteOneRow(sbHtml, strings, "<tr><td>", "</td><td>", "</td></tr>", HtmlEncode, columnFormats);
                WriteOneRow(sbCsv, strings, "", ",", "", CsvEncode, columnFormats);
            }
        ...
        private delegate string StringToString(string str,string format);

        private void WriteOneRow(StringBuilder sb, IEnumerable<string> strings, string startRow, string betweenCells, string endRow, StringToString encoder, List<string> rowFormats) {
            sb.Append(startRow);
            bool first = true;
            int i = 0;
            foreach (string s in strings) {
                ...
                sb.Append(encoder == null ? s : encoder(s,rowFormats[i++]));
                first = false;
            }
        ...
        private static string TextEncode(string text, string format)
        {
            if (text == null)
                return null;
        ...
        /// <returns></returns>
        private static string CsvEncode(string text, string format) {
            if (text == null)
                return null;
        ...
        private static string HtmlEncode(string text, string format) {
            if (text == null)
                return null;
