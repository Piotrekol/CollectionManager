...
public class OsuSite
{
    ...
    private List<string> GetUsernames(int startPage, int endPage, int startUserIndex, int endUserIndex)
    {
        ...
        using (WebClient client = new())
        {
            ...
            {
                _logger?.Invoke(string.Format(parsingPage, i, endPage), Convert.ToInt32(processedPages++ / (float)totalPages * 100));
                string pageContents = client.DownloadString(string.Format(UserPpRankingUrl, i));
            ...
            _logger?.Invoke(string.Format(parsingComplete, totalPages), Convert.ToInt32(processedPages / (float)totalPages * 100));
        }
    ...
    private List<string> GetUsernamesFromPage(string pageContents, int startUserIndex, int endUserIndex)
    {
        ...
        {
            MatchCollection match = Regex.Matches(pageContents, ".*?href=\".*?\\/users\\/(\\d+)\\/osu\"\n.*\n.*\n.*\n.*>\n(.*)\n.*<\\/a>");
            if (match.Count > 0)
            {
