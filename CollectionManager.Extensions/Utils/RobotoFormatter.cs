...
public static class RobotoFormatter
{
    ...
    private static Descriptor[] GetDescriptors(string format)
    {
        ...
        StringBuilder descriptorBuilder = new(format.Length);
        ...
        for (int i = 0; i < format.Length; i++)
        {
            ...
            if (isDescriptor || character == '{') //we found beginning
            {
                ...
                if (character != '}')
                {
                    if (character != '{')
                    {
                        _ = descriptorBuilder.Append(character);
                    }
                }
                else if (GetNextChar(i, format) != '}') //found an end (ignoring escaped closing)
                {
                    Descriptor descriptor = new(descriptorBuilder.ToString());
                    if (!list.Contains(descriptor))
                    {
                        list.Add(descriptor);
                    }

                    _ = descriptorBuilder.Remove(0, descriptorBuilder.Length);
                    isDescriptor = false;
                }
            }
        }
    ...
    private class Descriptor : IEquatable<Descriptor>
    {
        ...
        //---------------------------------------------------------
        public string ValueToString(object value) => string.Format(ToFormatString(), value);
        ...
        //---------------------------------------------------------
        public bool Equals(Descriptor other) => FullDescriptor.Equals(other.FullDescriptor);
    }
}