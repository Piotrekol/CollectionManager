...
namespace CollectionManagerExtensionsDll.Modules.API.osu
{
    public class OsuApi
    {
        ...
        public IList<ApiScore> GetUserBest(string username, PlayModes mode, int limit = 100)
        {
        ...
        private BeatmapExtension GetBeatmapResult(string url)
        {
            try
            {
                ...
                beatmap.TitleRoman = json["title"].ToString();
                beatmap.Creator = json["creator"].ToString();
                beatmap.ModPpStars.Add(0,
                    Math.Round(double.Parse(json["difficultyrating"].ToString(), CultureInfo.InvariantCulture), 2));
