...
public class BeatmapExporter
{
    ...
    public void ExportBeatmap(BeatmapSetExport beatmapSetExport)
    {
        try
        {
            var targetFilePath = beatmapSetExport.TargetFilePath(_SaveDirectory);
            ...
            else if (beatmapSetExport is LazerBeatmapSetExport lazerBeatmapSetExport)
            {
                ...
                {
                    var sourceFilePath = Path.Combine(_OsuSongsOrLazerFilesDirectory, setFile.RelativeRealmFilePath);
