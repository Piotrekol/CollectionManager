...
internal abstract class AudioPlayer : IMusicPlayer
{
    ...
    public void Seek(double position)
    {
        ...
        bool enoughLeft = TotalTime - position < 0.15;
        if (enoughLeft)
        {
            position -= 0.15;
        }
    ...
    public void SetVolume(float volume) => SoundVolume = volume;

    public virtual void SetSpeed(float speed) => throw new NotImplementedException("Base NAudio played does not support speed control.");
    ...
    public void StopPlayback()
    {
        lock (_lockingObject)
        {
            if (_waveOutDevice.PlaybackState == PlaybackState.Playing)
            {
                ...
                float oldVolume = _waveOutDevice.Volume;
    ...
    public void Dispose() => CleanAfterLastPlayback();
}